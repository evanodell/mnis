...
has_bom <- function(resp, encoding = "UTF-8") {
    if (inherits(resp, "response")) {
        F <- resp$content[1:4]
        switch(encoding, `UTF-8` = F[1] == as.raw(239) & F[2] == as.raw(187) & F[3] == as.raw(191), `UTF-16` = F[1] == as.raw(255) & F[2] == as.raw(254), `UTF-16BE` = F[1] == as.raw(254) & F[2] == as.raw(255), {
            message("Unsupported encoding")
            return(NA)
        })
    } else if (inherits(resp, "character")) {
        switch(encoding, `UTF-8` = grepl("^ï»¿", resp[1]), `UTF-16` = grepl("^ÿþ", resp[1]), `UTF-16BE` = grepl("^þÿ", resp[1]), {
            message("Unsupported encoding")
            return(NA)
        })
    } else {
