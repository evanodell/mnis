...
#' @param date1 The date to return the list of peers from. Defaults to current system date. Accepts character values in "YYYY-MM-DD" format, and objects of class Date, POSIXt, POSIXct, POSIXlt or anything else than can be coerced to a date with `as.Date()`.
#' @param date2 An optional query parameter. Accepts character values in "YYYY-MM-DD" format, and objects of class Date, POSIXt, POSIXct, POSIXlt or anything else than can be coerced to a date with `as.Date()`. If not NULL, the function returns a list of all peers in the House of Lords between date2 and date1. Defaults to NULL.
...
#' @export
#' @seealso [mnis_party_state()] [mnis_peers_on_date()]
#' @examples \dontrun{
#'
